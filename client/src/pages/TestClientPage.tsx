...
const STEPS = [
  "1. Create Site", "2. Approve Site", "3. List Cranes", "4. Assign Crane",
  "5. Assign Driver", "6. Record Attendance", "7. Request Document",
  "8. Submit Document", "9. Review Document",
];
  ...
  const handleRunWorkflow = async () => {
    ...
    const workflowLogger = (message: string, data?: any) => {
        ...
        if (message.startsWith('STEP')) {
          ...
          if (stepMatch) setCurrentStep(parseInt(stepMatch[1], 10));
        }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                return (
                  <div key={index} className="flex items-center mb-4 last:mb-0">
                    <div className="relative z-10">
                      <div className={cn("w-8 h-8 rounded-full flex items-center justify-center border-2", { "border-cyan-glow bg-cyan-900 animate-pulse": isCurrent, "border-green-500 bg-green-900": isCompleted, "border-red-500 bg-red-900 animate-ping": isFailed, "border-gray-600 bg-gray-800": status === 'pending' })}>
                        <span className="text-sm font-bold">{index + 1}</span>
                      </div>
                    </div>
                    <span className={cn("ml-4 text-sm font-medium", { "text-cyan-glow": isCurrent, "text-green-400": isCompleted, "text-red-400": isFailed, "text-gray-400": status === 'pending' })}>
                      {step}
                    </span>
                  </div>
                );
        ...
        <div className="lg:col-span-3 flex flex-col gap-8">
          <div className="text-center">
            <button onClick={handleRunWorkflow} disabled={isRunning} className={cn("px-8 py-4 text-lg font-bold uppercase tracking-widest text-white transition-all duration-300 rounded-lg", "bg-cyan-600 hover:bg-cyan-500 disabled:bg-gray-700 disabled:cursor-not-allowed", "shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/40", "transform hover:-translate-y-1", {"animate-pulse": !isRunning})}>
          ...
          <div className="bg-black bg-opacity-30 p-6 rounded-lg border border-cyan-glow/20 flex-grow">
            ...
            <div className="h-96 overflow-y-auto p-4 bg-black bg-opacity-50 rounded-lg border border-gray-700">
              {logs.map((log, index) => (
                <div key={index} className={cn("flex items-start mb-2", { 'text-red-400': log.type === 'error', 'text-green-400': log.type === 'success', 'text-gray-400': log.type === 'info' })}>
